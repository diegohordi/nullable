...
func TestFloat64_MarshalJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "should return the given float64",
			fields: fields{
				value: *nullable.NewFloat64(math.MaxFloat64),
			},
			want:    []byte(fmt.Sprintf("%v", math.MaxFloat64)),
			wantErr: false,
		},
		{
			name: "should marshal the given float64 from a struct",
			fields: fields{
				value: &struct {
					ID    int              `json:"id"`
...
func TestFloat64_Scan(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "should return a nullable float64 with 0 as value",
			fields: fields{
				value: nil,
			},
			want: nullable.Float64{
		...
		{
			name: "should return a nullable float64 with the given value as its value",
			fields: fields{
				value: 100,
			},
			want:    *nullable.NewFloat64(100),
			wantErr: false,
		},
		{
